### CoMMpass TTF CoxPH Spectra UAMS SBUK bootstrap production version 20210603
...
commpass_tf <- vroom("./CoMMpass_inclfollowup_final_SpectraData_20210601/spectra_scores-batch_variables-2021-06-01.csv")
colnames(commpass_tf) <- gsub(x=colnames(commpass_tf), pattern="PC",replacement="Sp")

# filter to only baseline samples and only select variables needed for this analysis
# divide each spectra var by SD 
cmps_tf_wid <- commpass_tf %>% 
  filter(smp_group=="Baseline") %>% 
  select(sample_id, ttctf1, censtf1, Sp1:Sp39) %>% 
  mutate(across(Sp1:Sp39, ~ .x/sd(.x)))

## uncomment to remove unscaled version of spectra with follow-up samples to reduce memory use 
# rm(commpass_tf)

## uncomment to write out SD scaled version of spectra with TTF data for baseline samples only for modeling
# write_csv(cmps_tf_wid, "commpass_spectra_baseline_tf_20210603.csv")
...
uams_results_sdata <- left_join(uams_results, 
                                dplyr::select(cmps_tf_wid, sample_id, ttctf1, censtf1), by="sample_id") 
uams_results_sdata$ua_group <- factor(uams_results_sdata$ua_group, levels = c("low","high"))
## actuarial HR
uams_act_hr <- act.haz(uams_results_sdata, "ttctf1", "censtf1", "ua_group")
...
## actuarial HR
sbuk_act_hr <- act.haz(sbuk_results_sdata, "ttctf1", "censtf1", "sb_group")

## write out results of these classifiers on all data
write_csv(bind_cols(uams_act_hr=uams_act_hr, sbuk_act_hr=sbuk_act_hr), 
          "commpass_tf_alldata_uams_sbuk_HRs_moregenes_20210603.csv") # summary results for HRs from UAMS and SBUK

## bootstrapping 
set.seed(5197)
brs <- 1000 # 1000 bootstrap replications 
## all spectra dataset 
cmps_tf <- cmps_tf_wid %>% dplyr::select(-sample_id)
cmps_class <- cmps_tf %>% dplyr::select(ttctf1, censtf1)
## store output
# list of sample_ids for bootstrap sample
bootlist_results <- matrix(nrow=nrow(cmps_tf), ncol=brs)
# C index all vars
...
cph_boot_linear_preds <- matrix(nrow=nrow(cmps_tf), ncol=brs)
cph_orig_linear_preds <- matrix(nrow=nrow(cmps_tf), ncol=brs)

# linear predictors from all sigp (p<0.05) vars CoxPH model (basis for classification)
cph_boot_linear_preds_sigp <- matrix(nrow=nrow(cmps_tf), ncol=brs)
cph_orig_linear_preds_sigp <- matrix(nrow=nrow(cmps_tf), ncol=brs)

# bootstrap outcome predictions for orig data (classes based on GMM threshold)
gmm2_orig_classes <- matrix(nrow=nrow(cmps_tf), ncol=brs) 
gmm2_orig_classes_sigp <- matrix(nrow=nrow(cmps_tf), ncol=brs)
...
for(b in 1:brs){
  ## using all vars
  boot_list <- sample(1:nrow(cmps_tf), size = nrow(cmps_tf), replace=T)
  bootlist_results[,b] <- cmps_tf_wid$sample_id[boot_list]
  boot_set <- cmps_tf[boot_list,]
  bs_class <- boot_set %>% dplyr::select(ttctf1, censtf1)
  temp_mod <- coxph(Surv(ttctf1, censtf1) ~ ., data=boot_set)
  ...
  # oob sample
  oob_set <- cmps_tf[-boot_list,]
  oob_class <- oob_set %>% dplyr::select(ttctf1, censtf1)
  oob_class$lp <- predict(temp_mod, oob_set %>% dplyr::select(contains("Sp")))
  cindex_oob_fh[b] <- 1-rcorr.cens(oob_class$lp, Surv(oob_class$ttctf1, oob_class$censtf1))[1]
  # original data
  cmps_class$lp <- predict(temp_mod, cmps_tf %>% dplyr::select(contains("Sp")))
  cph_orig_linear_preds[,b] <- cmps_class$lp
  cindex_b_orig_fh[b] <- 1-rcorr.cens(cmps_class$lp, Surv(cmps_class$ttctf1, cmps_class$censtf1))[1]
  ...
  sigp_mod <- coxph(Surv(ttctf1, censtf1) ~ ., 
                    data=dplyr::select(boot_set, ttctf1, censtf1, all_of(sigp_sps)))
  ...
  cindex_oob_sigp[b] <- 1-rcorr.cens(oob_class$sigp_lp, Surv(oob_class$ttctf1, oob_class$censtf1))[1]
  # original data
  cmps_class$sigp_lp <- predict(sigp_mod, cmps_tf %>% dplyr::select(all_of(sigp_sps)))
  cph_orig_linear_preds_sigp[,b] <- cmps_class$sigp_lp
  cindex_b_orig_sigp[b] <- 1-rcorr.cens(cmps_class$sigp_lp, Surv(cmps_class$ttctf1, cmps_class$censtf1))[1]
  ...
  ## actuarial HR
  bs_gmm2_act_hr[b] <- act.haz(bs_class, "ttctf1", "censtf1", "gmm2_class")
  ## on original data
  ...
  ## actuarial HR
  or_gmm2_act_hr[b] <- act.haz(cmps_class, "ttctf1", "censtf1", "gmm2_class")
  ## on oob sample 
  ...
  if(table(oob_class$gmm2_class)["low"]==0 | table(oob_class$gmm2_class)["high"]==0){
    ...
    oob_gmm2_act_hr[b] <- act.haz(oob_class, "ttctf1", "censtf1", "gmm2_class")
  }
  ...
  ## actuarial HR
  bs_sigp_act_hr[b] <- act.haz(bs_class, "ttctf1", "censtf1", "sigp_class")
  ## on original data
  ...
  ## actuarial HR
  or_sigp_act_hr[b] <- act.haz(cmps_class, "ttctf1", "censtf1", "sigp_class")
  ## on oob sample 
  ...
  if(table(oob_class$sigp_class)["low"]==0 | table(oob_class$sigp_class)["high"]==0){
    ...
    oob_sigp_act_hr[b] <- act.haz(oob_class, "ttctf1", "censtf1", "sigp_class")
  }
  ...
  uams_hr_boot[b] <- act.haz(uams_sdata, "ttctf1", "censtf1", "ua_group")
  # HRs on original data 
  uams_hr_orig[b] <- act.haz(uams_results_sdata_temp, "ttctf1", "censtf1", "ua_gr_orig")
  # HRs on oob data
  uams_hr_oob[b] <- act.haz(uams_results_sdata_oob, "ttctf1", "censtf1", "ua_gr_oob")

  ## SBUK with fixed betas
  # HRs on bootstrap data 
  sbuk_hr_boot[b] <- act.haz(sbuk_sdata, "ttctf1", "censtf1", "sb_group")
  # HRs on original data 
  sbuk_hr_orig[b] <- act.haz(sbuk_results_sdata_temp, "ttctf1", "censtf1", "sb_gr_orig")
  # HRs on oob data
  sbuk_hr_oob[b] <- act.haz(sbuk_results_sdata_oob, "ttctf1", "censtf1", "sb_gr_oob")
}
...
cph_orig_linear_preds_alls <- data.frame(sample_id=cmps_tf_wid$sample_id, cph_orig_linear_preds)
...
cph_orig_linear_preds_sigp <- data.frame(sample_id=cmps_tf_wid$sample_id, cph_orig_linear_preds_sigp)
...
gmm2_orig_classes <- data.frame(sample_id=cmps_tf_wid$sample_id, gmm2_orig_classes)
colnames(gmm2_orig_classes) <- c("sample_id", paste0("boot", 1:brs))
gmm2_orig_classes_sigp <- data.frame(sample_id=cmps_tf_wid$sample_id, gmm2_orig_classes_sigp)
...
write_csv(bootlist_results, "commpass_tf_bootlist_results_20210603.csv")
write_csv(cindex_results, "commpass_tf_cindex_results_20210603.csv")

write_csv(cph_boot_linear_preds_alls, "commpass_tf_cph_boot_linear_preds_alls_20210603.csv")
write_csv(cph_orig_linear_preds_alls, "commpass_tf_cph_orig_linear_preds_alls_20210603.csv")
write_csv(cph_boot_linear_preds_sigp, "commpass_tf_cph_boot_linear_preds_sigp_20210603.csv")
write_csv(cph_orig_linear_preds_sigp, "commpass_tf_cph_orig_linear_preds_sigp_20210603.csv")

write_csv(boot_pvals_alls, "commpass_tf_boot_pvals_alls_20210603.csv")
write_csv(boot_pvals_sigp, "commpass_tf_boot_pvals_sigp_20210603.csv")

write_csv(all_thresh_results, "commpass_tf_all_thresh_results_20210603.csv")
write_csv(uams_sbuk_def_thresh, "commpass_uams_sbuk_thresh_results_20210603.csv")

write_csv(gmm2_orig_classes, "commpass_tf_gmm2_alls_orig_classes_20210603.csv")
write_csv(gmm2_orig_classes_sigp, "commpass_tf_gmm2_sigp_orig_classes_20210603.csv")

write_csv(spectra_actuarial_HR_res, "commpass_tf_spectra_actuarial_HR_res_20210603.csv")

write_csv(uams_sbuk_act_HR, "commpass_tf_uams_sbuk_act_HR_res_mg_20210603.csv") 
